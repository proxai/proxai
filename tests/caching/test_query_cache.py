...
def _save_shard_manager(path, records=None):
  save_shard_manager = query_cache.ShardManager(
      path=path, shard_count=_SHARD_COUNT, response_per_file=_RESPONSE_PER_FILE
  )
  if records:
...
def _create_cache_record(
    prompt, responses, shard_id, call_count, all_values=False
) -> types.CacheRecord:
  ...
  return (
      query_record.hash_value, cache_record, light_record,
...
def _get_example_records(shard_dir=None, shard_count=None):
  ...
  records = [
      _create_cache_record(prompt='p1', responses=[], shard_id=0, call_count=0),
  ...
  (
      cache_records, light_cache_records, enc_cache_records,
      enc_light_cache_records
  ) = _generate_vals(records, write_to_shard=(shard_dir is not None))
  ...
  (
      ...
      enc_cor_light_cache_records
  ) = _generate_vals(records)
...
def _check_light_cache_records_file(filepath, expected):
  data = {}
  with open(filepath) as f:
    for line in f:
  ...
  for prompt in prompts:
    for hash, cache_record in records['cache_records'].items():
      if cache_record.query_record.prompt == prompt:
        ...
        light_cache_records.append(
            copy.deepcopy(records['light_cache_records'][hash])
        )
        enc_light_cache_records.append(
            copy.deepcopy(records['enc_light_cache_records'][hash])
        )
  return {
      'cache_records': cache_records,
      'enc_cache_records': enc_cache_records,
...
def _unpack_records(records):
  hash_1 = copy.deepcopy(_get_hash_from_prompt(prompt='p1', records=records))
  light_1 = copy.deepcopy(records['light_cache_records'][hash_1])
  enc_light_1 = copy.deepcopy(records['enc_light_cache_records'][hash_1])
  hash_2 = copy.deepcopy(_get_hash_from_prompt(prompt='p2', records=records))
  light_2 = copy.deepcopy(records['light_cache_records'][hash_2])
  enc_light_2 = copy.deepcopy(records['enc_light_cache_records'][hash_2])
  hash_3 = copy.deepcopy(_get_hash_from_prompt(prompt='p3', records=records))
  light_3 = copy.deepcopy(records['light_cache_records'][hash_3])
  enc_light_3 = copy.deepcopy(records['enc_light_cache_records'][hash_3])
  hash_4 = copy.deepcopy(_get_hash_from_prompt(prompt='p4', records=records))
  light_4 = copy.deepcopy(records['light_cache_records'][hash_4])
  enc_light_4 = copy.deepcopy(records['enc_light_cache_records'][hash_4])
  return (
      hash_1, light_1, enc_light_1, hash_2, light_2, enc_light_2, hash_3,
      light_3, enc_light_3, hash_4, light_4, enc_light_4
  )


def _print_state(shard_manager: query_cache.ShardManager, records):
  names = {}
  for i in range(1, 10):
    val = types.QueryRecord(prompt=f'p{i}')
    names[hash_serializer.get_query_record_hash(query_record=val)] = f'p{i}'
...
class TestBaseQueryCache:
  ...
  def test_get_cache_size(self):
    ...
    light_cache_record_size = query_cache._get_cache_size(light_cache_record)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      query_cache_manager_params = query_cache.QueryCacheManagerParams(
          cache_options=types.CacheOptions(
              cache_path=temp_dir, clear_query_cache_on_connect=False
      ...
      query_cache_manager_params = query_cache.QueryCacheManagerParams(
          cache_options=types.CacheOptions(
              cache_path=temp_dir, clear_query_cache_on_connect=True
          ), shard_count=_SHARD_COUNT, response_per_file=_RESPONSE_PER_FILE,
          cache_response_size=10
      )
      ...
      _check_shard_manager_state(
          query_cache_manager_3._shard_manager, light_cache_records={},
          enc_light_cache_records={}, map_shard_to_cache={},
          shard_active_count={
              0: 0,
              1: 0,
              2: 0,
...
class TestShardManager:
  ...
  def test_shard_names(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      assert (
          shard_manager.backlog_shard_path == temp_dir +
          '/shard_backlog_00003.jsonl'
      )
      assert (
          shard_manager.light_cache_records_path == temp_dir +
          '/light_cache_records_00003.json'
      )
      ...
      assert (
          shard_manager.backlog_shard_path == temp_dir +
          '/shard_backlog_00001.jsonl'
      )
      assert (
          shard_manager.light_cache_records_path == temp_dir +
          '/light_cache_records_00001.json'
      )
  ...
  def test_update_cache_record(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      hash_1, record_1, light_1, _, enc_light_1 = _create_cache_record(
          prompt='p1', responses=[], shard_id=0, call_count=1, all_values=True
      )
      shard_manager._update_cache_record(record_1)
      _check_shard_manager_state(
          shard_manager, light_cache_records={hash_1: light_1},
          enc_light_cache_records={hash_1: enc_light_1},
          map_shard_to_cache={0: {hash_1}}, shard_active_count={
              0: 1,
              1: 0,
              2: 0,
              'backlog': 0
      ...
      hash_1, record_1, light_1, _, enc_light_1 = _create_cache_record(
          prompt='p1', responses=[], shard_id=2, call_count=1, all_values=True
      )
      shard_manager._update_cache_record(record_1)
      _check_shard_manager_state(
          shard_manager, light_cache_records={
      ...
      hash_2, record_2, light_2, _, enc_light_2 = _create_cache_record(
          prompt='p2', responses=['r2', 'r3', 'r4'], shard_id=0, call_count=3,
          all_values=True
      )
      shard_manager._update_cache_record(record_2)
      _check_shard_manager_state(
  ...
  def test_save_load_light_cache_records(self):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      (
          hash_1, light_1, enc_light_1, hash_2, light_2, enc_light_2, hash_3,
          light_3, enc_light_3, hash_4, light_4, enc_light_4
      ) = _unpack_records(records=records)
      ...
      _check_shard_manager_state(
          load_shard_manager_3, light_cache_records={},
          enc_light_cache_records={}, enc_light_cache_records_backup={},
          map_shard_to_cache={}, shard_active_count={
              0: 0,
              1: 0,
              2: 0,
              'backlog': 0
  ...
  def test_move_backlog_to_shard(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      shard_manager._move_backlog_to_shard(shard_id=1)
      light_4.shard_id = 1
      enc_light_4 = type_serializer.encode_light_cache_record(light_4)
      _check_shard_manager_state(
          shard_manager, light_cache_records={
              hash_1: light_1,
              hash_2: light_2,
  ...
  def test_add_to_backlog(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      enc_light_5 = type_serializer.encode_light_cache_record(light_5)
      _check_shard_manager_state(
          shard_manager, light_cache_records={
              hash_1: light_1,
              hash_2: light_2,
  ...
  def test_get_cache_record(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      assert shard_manager.get_cache_record(hash_value) == records[
          'cache_records'][hash_value]

      # Test no hash value
      query_record = types.QueryRecord(prompt='p1')
      assert shard_manager.get_cache_record(query_record) == records[
          'cache_records'][_get_hash_from_prompt(prompt='p1', records=records)]
  ...
  def test_delete_record(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      _check_shard_manager_state(
          shard_manager, light_cache_records={hash_4: light_4},
          enc_light_cache_records={hash_4: enc_light_4},
          map_shard_to_cache={'backlog': {hash_4}}, shard_active_count={
              0: 0,
              1: 0,
              2: 0,
              'backlog': 2
      ...
      _check_shard_manager_state(
          shard_manager, light_cache_records={}, enc_light_cache_records={},
          map_shard_to_cache={}, shard_active_count={
              0: 0,
              1: 0,
              2: 0,
  ...
  def test_save_record(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      enc_light_6 = type_serializer.encode_light_cache_record(light_6)
      _check_shard_manager_state(
          shard_manager, light_cache_records={
              hash_1: light_1,
              hash_4: light_4,
      ...
      enc_light_1 = type_serializer.encode_light_cache_record(light_1)
      light_6.shard_id = 0
      enc_light_6 = type_serializer.encode_light_cache_record(light_6)
      _check_shard_manager_state(
          shard_manager, light_cache_records={
...
class TestQueryCache:
  ...
  def test_push_record_heap_existing_path(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      records = _get_example_records(
          shard_dir=cache_path, shard_count=_SHARD_COUNT
      )
      _save_shard_manager(
  ...
  def test_cache(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      _check_record_heap(
          query_cache_manager,
          [query_record_1.hash_value, query_record_2.hash_value]
      )
      _check_shard_manager_state(
          query_cache_manager._shard_manager, shard_active_count={
              0: 4,
      ...
      _check_record_heap(
          query_cache_manager, [
              query_record_1.hash_value, query_record_3.hash_value,
  ...
  def test_one_shard(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      query_cache_manager_params = query_cache.QueryCacheManagerParams(
          cache_options=types.CacheOptions(
              cache_path=temp_dir, unique_response_limit=3
          ), shard_count=1, response_per_file=_RESPONSE_PER_FILE,
          cache_response_size=10
      )
      query_cache_manager = query_cache.QueryCacheManager(
          init_from_params=query_cache_manager_params
...
class TestQueryCacheState:
  ...
  def test_invalid_combinations(self):
    with pytest.raises(ValueError):
      query_cache_manager_params = query_cache.QueryCacheManagerParams(
          ...
          shard_count=1, response_per_file=1, cache_response_size=10
      )
  ...
  def test_init_default_values(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      assert (query_cache_manager.cache_options.cache_path == temp_dir)
      assert (query_cache_manager.shard_count == default_values.shard_count)
      assert (
  ...
  def test_not_working_state_calls(self):
    with tempfile.TemporaryDirectory():
      ...
      with pytest.raises(
          ValueError, match=
          'QueryCacheManager status is QueryCacheManagerStatus.CACHE_PATH_NOT_FOUND'
      ):
