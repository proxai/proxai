  ...
  return [
      {
          'response':
              types.Response(
                  type=types.OutputFormatType.TEXT, value='Test response'
              )
      },
      {
          'error': 'Test error'
...
class TestLogging:
  ...
  def test_write_log(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      logging_utils._write_log(
          logging_options=logging_options, file_name=test_filename,
          data=test_data
      )
  ...
  def test_log_logging_record_stdout_only(self):
    ...
    logging_record = types.LoggingRecord(**_get_logging_record_examples()[2])
  ...
  def test_log_message(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      logging_utils.log_message(
          logging_options=logging_options, message=test_message,
          type=types.LoggingType.ERROR, query_record=query_record
      )

      assert os.path.exists(
          os.path.join(temp_dir, logging_utils.ERROR_LOGGING_FILE_NAME)
      )
      assert os.path.exists(
          os.path.join(temp_dir, logging_utils.MERGED_LOGGING_FILE_NAME)
      )
      ...
      assert os.path.exists(
          os.path.join(temp_dir, logging_utils.WARNING_LOGGING_FILE_NAME)
      )
      ...
      assert os.path.exists(
          os.path.join(temp_dir, logging_utils.INFO_LOGGING_FILE_NAME)
      )
  ...
  def test_log_proxdash_message(self):
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      logging_utils.log_proxdash_message(
          logging_options=logging_options, proxdash_options=proxdash_options,
          message=test_message, type=types.LoggingType.ERROR,
      ...
      assert os.path.exists(
          os.path.join(temp_dir, logging_utils.PROXDASH_LOGGING_FILE_NAME)
      )

      # Test WARNING type
      logging_utils.log_proxdash_message(
          logging_options=logging_options, proxdash_options=proxdash_options,
          message=test_message, type=types.LoggingType.WARNING
      )
