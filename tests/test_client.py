...
class TestProxAIClientGenerateText:
  ...
  def test_generate_text_with_extensive_return(self):
    px_client = _create_test_client()
    response = px_client.generate_text(prompt="hello", extensive_return=True)
...
class TestModelConnector:
  """Test ModelConnector class for model discovery."""

  def test_model_connector_init_with_client_getter(self):
    px_client = _create_client_from_params()
    connector = client.ModelConnector(lambda: px_client)
    assert connector._client_getter() is px_client

  def test_client_models_property_returns_model_connector(self):
    px_client = _create_client_from_params()
    assert isinstance(px_client.models, client.ModelConnector)

  def test_client_models_property_returns_same_instance(self):
    px_client = _create_client_from_params()
    models1 = px_client.models
    models2 = px_client.models
    assert models1 is models2

  def test_client_models_bound_to_client(self):
    px_client = _create_client_from_params()
    assert px_client.models._client_getter() is px_client

  def test_model_connector_list_models(self):
    px_client = _create_client_from_params()
    models = px_client.models.list_models()
    assert isinstance(models, list)
    assert len(models) > 0

  def test_model_connector_list_providers(self):
    px_client = _create_client_from_params()
    providers = px_client.models.list_providers()
    assert isinstance(providers, list)
    assert len(providers) > 0

  def test_model_connector_get_model(self):
    px_client = _create_client_from_params()
    model = px_client.models.get_model("mock_provider", "mock_model")
    assert model.provider == "mock_provider"
    assert model.model == "mock_model"

  def test_separate_clients_have_separate_model_connectors(self):
    px_client1 = _create_client_from_params()
    px_client2 = _create_client_from_params()
